...
from bikmeans import biKMeans,KMeans
#训练集
...
for i in range(len(y_pred)):
    if y_true[i]==int(y_map[i]):
        ct+=1
        okmap[int(y_pred[i])]=y_true[i]#获取预测标签所对应的真实标签
...
pca=pca.fit(sample_test)
array2=pca.transform(sample_test)
# 获取质心

centroid = km.centroids
...
for i in range(len(y_testpred)):
    if int(okmap[int(y_testpred[i])])==int(y_testtrue[i]):
        countok+=1
