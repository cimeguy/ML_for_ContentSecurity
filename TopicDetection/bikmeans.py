...
class KMeans(object):
    ...
    def __init__(self,n_clusters=6,initCent='random',max_iter=300):
        #initCent质心初始化方式，可选"random"或指定一个具体的array,默认random，即随机初始化
        # max_iter:最大迭代次数，可以更快停止算法
        if hasattr(initCent, '__array__'):
            n_clusters = initCent.shape[0]#聚类个数，即k
...
class biKMeans(object):
    def __init__(self,n_clusters=6):
        self.n_clusters = n_clusters#簇的个数
        self.centroids = None#质心
        self.clusterAssment = None#用于保存簇信息
        self.labels = None#标签
        self.sse = None#SSE
    #计算两点的欧式距离
    def _distEclud(self, vecA, vecB):
    ...
    def fit(self,X):
        ...
        centroid0 = np.mean(X, axis=0).tolist()#初始质点
        centList =[centroid0]
